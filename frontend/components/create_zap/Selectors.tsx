...
export function SolanaSelector({ setMetadata }: { setMetadata: (params: any) => void }) {
  ...
  return (
    <div className="text-white space-y-6 p-6 bg-[#1c1c1c] rounded-lg">
      <div>
        <Input label="To" type="text" placeholder="Enter email" onChange={(e) => setAddress(e.target.value)} />
      </div>
      <div>
        <Input label="Solana" type="text" placeholder="Enter the number SOL USDC" onChange={(e) => setAmount(e.target.value)} />
      </div>
      <div className="pt-4">
