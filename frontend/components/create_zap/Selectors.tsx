...
export function EmailSelector({ setMetadata }: { setMetadata: (params: any) => void }) {
  ...
  return (
    <div className="text-white space-y-6 p-6 bg-[#1c1c1c] rounded-lg ">
      <div>
        <Input label="To" placeholder="Enter recipient's email as {comment.email}" onChange={(e: any) => setEmail(e.target.value)} />
      </div>
      <div>
...
export function InrSelector ({ setMetadata }: { setMetadata: (params: any) => void }) {
  ...
  return <div className="text-white space-y-6 p-6 rounded-lg bg-[#1c1c1c] shadow-md">
    <div>
      <Input label="To" placeholder="Enter recipient's email as {comment.email}" type="text" onChange={(e: any) => setAddress(e.target.value)}></Input>
    </div>
    <div>
      <Input label="Amount" placeholder="Enter the INR as {comment.amount}" type="text" onChange={(e: any) => setAmount(e.target.value)}></Input>
    </div>
    <div className="pt-4">
      <Button onClick={() => {
